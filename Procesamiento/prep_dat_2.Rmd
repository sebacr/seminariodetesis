...
pacman::p_load(dplyr, sjmisc, car, sjlabelled, stargazer,kableExtra,corrplot,sessioninfo,readxl,pander,xtable, tidyverse, extrafont, ggplot2, forcats, ggpubr, naniar,haven, devtools,summarytools,webshot,poLCA)
...
proc_cep_2009$pem_ambicion <- set_label(x = proc_cep_2009$pem_ambicion,label = "Percepcion de merito ambición")
...
proc_cep_2019$pem_ambicion <- set_label(x = proc_cep_2019$pem_ambicion,label = "Percepcion de merito ambición")
...
proc_data_2019$pmerit <- (proc_data_2019$percep_inteligencia+proc_data_2019$pem_ambicion+proc_data_2019$percep_esfuerzos)/3

proc_data_2019$pmerit  <- set_label(x = proc_data_2019$pmerit, label = "Percepción meritocracia promedio")

data_percepcion<-proc_data_2019 %>% dplyr::select(pem_ambicion,percep_inteligencia,percep_esfuerzos)
...
proc_data<-subset(proc_data, select = -c(pem_ambicion))
...
proc_data %>% 
  dplyr::select(starts_with("Att_")) %>%  
  poLCA(f, ., nclass = 3, nrep = 6, graphs = TRUE, maxiter = 20000) -> LCA_3clases # BIC = 21464.48
```
